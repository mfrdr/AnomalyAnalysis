...
class EdgeStructureLearner(nn.Module):
    def __init__(self, num_nodes, max_num_edges, dim, static_feat, device='cuda', alpha1=0.1, alpha2=2.0,
        ...
        if not isinstance(static_feat, torch.Tensor):
            static_feat = torch.from_numpy(static_feat)

        self.static_feat = static_feat.to(dtype=torch.float32, device=device, non_blocking=True)
