...
class X_Dataset(torch.utils.data.Dataset):
...
def to_dataloaders_oc(cmip5, soda, godas, batch_size, valid_split=0, validation='SODA', verbose=True,
    ...
    trainset = X_Dataset(trainX, trainY)
    valset = X_Dataset(validX, validY) if validX is not None else []
    testset = X_Dataset(godasX, godas[1])
...
def to_dataloaders_atm(cmip5, noaa, era5, batch_size, valid_split=0, validation='SODA', verbose=True,
                   concat_cmip5_and_soda=True, shuffle_training=True):
    """
     n - length of time series (i.e. dataset size)
     m - number of nodes/grid cells (33 if using exactly the ONI region)
    ...
    if concat_cmip5_and_soda:  # instead of transfer, concat the cmip5 and noaa data
        if validation.lower() == 'cmip5':
            first_val = min(len(era5[1]) * 2, 600)
            cmip5_trainX, cmip5_trainY = cmip5X[:-first_val], cmip5[1][:-first_val]
            validX, validY = cmip5X[-first_val:], cmip5[1][-first_val:]
            noaa_trainX, noaa_trainY = noaaX, noaa[1]
        elif 'soda' in validation.lower():
            cmip5_trainX, cmip5_trainY = cmip5X, cmip5[1]
            if valid_split > 0:
                first_val = int(valid_split * len(noaaX))
                noaa_trainX, noaa_trainY = noaaX[:-first_val], noaa[1][:-first_val]
                if validation.lower() == 'soda':
                    validX, validY = noaaX[-first_val:], noaa[1][-first_val:]
                else:
                    validX, validY = noaaX, noaa[1]
            else:  # without val. set, just return the NOAA set
                noaa_trainX, noaa_trainY = noaaX, noaa[1]
                validX, validY = noaaX, noaa[1]
        else:
            raise ValueError('Validation dataset must be either CMIP5 or NOAA')
        trainX = np.concatenate((cmip5_trainX, noaa_trainX), axis=0)
        trainY = np.concatenate((cmip5_trainY, noaa_trainY), axis=0)
    else:
        print("Only SODA for training!")
        first_val = int(valid_split * len(noaa[0]))
        trainX, trainY = noaaX[:-first_val], noaa[1][:-first_val]
        validX, validY = noaaX[-first_val:], noaa[1][-first_val:]

    trainset = X_Dataset(trainX, trainY)
    valset = X_Dataset(validX, validY) if validX is not None else []
    testset = X_Dataset(era5X, era5[1])
...
def reformat_cnn_data(lead_months=3, window=3, use_heat_content=True,
                      lon_min=0, lon_max=360,
                      lat_min=-55, lat_max=60,
                      data_dir="data/",
                      sample_file='CMIP5.input.36mn.1861_2001_og.nc',  # Input of training set
                      # label_file='CMIP5.label.mldgron.12mn_3mv.1863_2003.nc',  # Label of training set MLD
                      label_file='CMIP5.label.nino34.12mn_3mv.1863_2003.nc',  # Label of training set Nino
                      sst_key="sst",
    ...
    # print(data["sst1"].shape, labels["pr"].shape)
    ...
    # print(Y_all_target_mons.shape)
    # print("Filtered region lat size:", len(filtered_region["lat"]))
    # print("Expected lat size:", lat_sz)
...
def load_cnn_data(lead_months=3, window=3, use_heat_content=True,
                  lon_min=0, lon_max=359,
                  lat_min=-55, lat_max=60,
                  data_dir="data/",
                  cmip5_data='CMIP5.input.36mn.1861_2001_og.nc',  # Input of CMIP5 training set (add _w when  doing ElNino)
                  cmip5_label='CMIP5.label.nino34.12mn_3mv.1863_2003.nc',  # El Nino
                  # cmip5_label='CMIP5.label.mldgron.12mn_3mv.1863_2003.nc',  # Cold Blob
                  
                  soda_data='SODA.input.36mn.1871_1970.nc',  # Input of SODA training set
                  soda_label='SODA.label.nino34.12mn_3mv.1873_1972.nc',  # El Nino
                  # soda_label='SODA.label.mldgron.12mn_3mv.1873_1972.nc',  # Cold Blob
                  
                  # godas_data='GODAS.input.36mn.1980_2012.nc',  # Input of GODAS training set mld
                  godas_data='GODAS.input.36mn.1980_2015.nc',  # Input of GODAS training set nino
                  godas_label='GODAS.label.12mn_3mv.1982_2017.nc',  # El Nino
                  # godas_label='GODAS.label.mldgron.12mn_3mv.1982_2014.nc',  # Cold Blob
    ...
    # print("SHAPES BEFORE:", cmip5.shape, SODA.shape, GODAS.shape)
    cmip5, SODA, GODAS = cmip5[:, :, :, final_mask], SODA[:, :, :, final_mask], GODAS[:, :, :, final_mask]
    # print("SHAPES:", cmip5.shape, SODA.shape, GODAS.shape)
    if add_index_node:
...
def reformat_atm_data(lead_months=3, window=3,
                      ...
                      label_file='CMIP5.label.nino34.12mn_3mv.1863_2003.nc',  # El Nino
                      # label_file='CMIP5.label.mldgron.12mn_3mv.1863_2003.nc',  # Cold Blob
                      sp_key="sp",
                      get_valid_nodes_mask=False,
                      get_valid_coordinates=False
                      ):
...
def load_atm_data(lead_months=3, window=3,
                  ...
                  lat_min=-55, lat_max=60,
                  data_dir="data/",
                  cmip5_data='CMIP5.input.36mn_3mv.1861_2001.nc',  # Input of CMIP5 training set
                  cmip5_label='CMIP5.label.nino34.12mn_3mv.1863_2003.nc',  # El Nino
                  # cmip5_label='CMIP5.label.mldgron.12mn_3mv.1863_2003.nc',  # Cold Blob
                  noaa_data='NOAA.input.36mn_3mv.1871_1970.nc',  # Input of NOAA training set
                  noaa_label='NOAA.label.nino34.12mn_3mv.1873_1972.nc',  # El Nino
                  # noaa_label='NOAA.label.mldgron.12mn_3mv.1873_1972.nc',  # Cold Blob
                  era5_data='ERA5.input.36mn_3mv.1980_2015.nc',  # Input of ERA5 training set
                  era5_label='ERA5.label.12mn_3mv.1982_2017.nc',  # El Nino
                  # era5_label='ERA5.label.mldgron.12mn_3mv.1982_2014.nc',  # Cold Blob
    ...
    # print("SHAPES:", cmip5.shape, NOAA.shape, ERA5.shape, cmip5_Y.shape, NOAA_Y.shape, cmip5_Y.shape)
    if add_index_node:
