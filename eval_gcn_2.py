...
def reload_all(model_dir, ens_dict, checkpoint_IDs=None, device='cuda', data_dir='./Data/'):
    ...
    from graphino.training_2 import evaluate, get_dataloaders
    print(model_dir)
    if checkpoint_IDs is None:
        checkpoint_IDs = ['last_2']

    if os.path.isfile(os.path.join(model_dir, 'last_model.pkl')):
        mdl_file = os.path.join(model_dir, 'last_model.pkl')
    elif os.path.isfile(os.path.join(model_dir, 'last_2_model.pkl')):
        mdl_file = os.path.join(model_dir, 'last_2_model.pkl')
    else:
    ...
    (adj_atm, adj_oc, static_feats_atm, static_feats_oc, _), (_, valloader, testloader) = get_dataloaders(params, net_params)
    model = GCN_2(net_params, static_feat_atm=static_feats_atm, static_feat_oc=static_feats_oc, 
                  adj_atm=adj_atm, adj_oc=adj_oc, verbose=False, device=device)
    Y = None
...
if __name__ == '__main__':
    ...
    parser.add_argument("--horizon", default=3, type=int)  # number of lead months
    parser.add_argument("--type", default='last_2', type=str)
    ...
    print("Entering ensenmble performance evaluation")
